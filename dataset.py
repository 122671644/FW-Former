...
import torch
from utils import is_png_file, load_img, load_mask, Augment_RGB_torch, load_test_img, load_test_mask
import random
...
class DataLoaderVal(Dataset):
    ...
    def __getitem__(self, index):
        tar_index = index % self.tar_size
        clean = torch.from_numpy(np.float32(load_test_img(self.clean_filenames[tar_index])))
        noisy = torch.from_numpy(np.float32(load_test_img(self.noisy_filenames[tar_index])))
        mask = torch.from_numpy(np.float32(load_test_mask(self.mask_filenames[tar_index])))
...
class DataLoaderTest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return noisy, mask,  noisy_filename